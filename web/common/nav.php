</head>
<body class="bg-dark h-100 m-0">
        <nav class="navbar navbar-expand-md bg-info navbar-dark text-dark border-bottom box-shadow mb-3">
            <div class="container">
                <a class="navbar-brand text-dark" href="index.php">CSE 341 Software Engineering I</a>
                <button class="navbar-toggler" type="button">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="navbar-collapse d-md-inline-flex flex-md-row-reverse">
                    <ul id="link-nav" class="navbar-nav flex-grow-1 d-md-inline-flex">
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="<?php echo $_SERVER['DOCUMENT_ROOT'] . '/index.php';?>">Home</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-dark" href="<?php echo $_SERVER['DOCUMENT_ROOT'] . '/assignments.php';?>">Assignments</a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>

    