 ...
 function getWishes() {
   $db = connectMyBooks();
    if (!$db) {
      ...
      $statement = $db->query('SELECT * FROM book_title WHERE own_wish_list = TRUE AND read_wish_list = TRUE ORDER BY title_of_book');
      $results = $statement->fetchAll(PDO::FETCH_ASSOC);
      return $results;
   }
}
...
function displayCatalog($catalog) {
 ...
 function displayWishList($wish) {
      ...
      foreach ($catalog as $book) {
         ...
         $bookList .= '<td class="pl-5">' . getReadList($book['']) . '</td>';
         $bookList .= '<td class="pl-5">' . getWishOwn($book['']) . '</td>';
         $bookList .= '</tr>';
      }
      $bookList .= '</tbody>';
      return $bookList;
