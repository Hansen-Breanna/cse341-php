...
function getCatalogAuthor($first_name, $last_name, $id) {
 ...
 function getReviews() {
   $db = connectMyBooks();
   $stmt = $db->query('SELECT r.review, r.rating, a.first_name, a.middle_name, a.last_name, b.title_of_book, lu.username FROM reviews r INNER JOIN book_title b ON r.book_title_id = b.id INNER JOIN library_user lu ON r.library_user_id = lu.id INNER JOIN author a ON a.id = b.author_id ORDER BY b.title_of_book');
   $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
   return $results;
}
