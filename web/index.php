<!-- Nav -->
<?php include 'common/nav.php'; ?>

<!-- Header -->
<?php include 'common/header.php'; ?>

              <h1 class="display-2">Breanna Hansen</h1>
          </div>
      </div>
    </header>
    
    <main class="mb-5">
      <div class="container">
            <div class="row d-md-flex">
                <div id="box-1" class="bg-light shadow-lg rounded box p-2 mt-4 offset-2 col-8 offset-md-0 col-md d-flex flex-column">
                  <h2 class="text-center display-5 py-3">Outdoors</h2>
                  <img class="offset-1 col-10" src="images/outdoors.jpg" alt="Mountains behind a lake and trees">
                  <p class="py-3 offset-1 col-10">I love to enjoy the outdoors</p>
                </div>
                <div id="box-2" class="bg-light shadow-lg rounded box p-2 mt-4 offset-2 col-8 offset-md-1 col-md">
                  <h2 class="text-center display-5 py-3">Games</h2>
                  <img class="offset-1 col-10" src="images/dominion-strategy-game.jpg" alt="Dominion Strategy Games">
                  <p class="py-3 offset-1 col-10">My kids and I love to play games together.</p>
                </div>
                <div id="box-3" class="bg-light shadow-lg rounded box p-2 mt-4 offset-2 col-8 offset-md-1 col-md">
                  <h2 class="text-center display-5 py-3">Reading</h2>
                  <img class="offset-1 col-10" src="images/library.jpg" alt="Large public library">
                  <p class="py-3 offset-1 col-10">I love to read.</p>
                </div>
            </div> 
            <div class="row col-6 offset-3 col-md-8 offset-md-2">
                <button type="button" class="col-md btn btn-lg bg-info mt-5 text-dark shadow-lg">Assignments</button>
            </div> 
        </div>
      </main>

<!-- Footer -->
<?php include 'common/footer.php'; ?>